...
(function () {
  ...
  var T_TEAM = '<div class="view-anim">'
    ...
    + '<span v-if="s.open_id" style="font-size:11px;color:#047857;background:#ecfdf5;padding:1px 8px;border-radius:10px">✓ 已绑定飞书</span>'
    + '<span v-else style="font-size:11px;color:#b45309;background:#fef3c7;padding:1px 8px;border-radius:10px">未绑定，无法用飞书看客户</span>'
    + '</div></td>'
    + '<td class="row-action">'
  ...
  var T_SRC = '<div class="view-anim">'
    ...
    + '<section class="card">'
    + '<div class="card-head">'
    + '<div><h3 style="display:flex;align-items:center;gap:8px"><span style="color:#6366f1" v-html="ico.sparkles"></span> 2. 大模型接入与切换 (LLM)</h3>'
    + '<span class="card-sub">支持 DeepSeek / Kimi / 小米 MiMo 等一切 OpenAI 兼容端点 · 填入 Key 即可一键启用</span></div>'
    + '<div style="display:flex;gap:8px">'
    + '<button class="btn btn-primary" style="background:linear-gradient(135deg,#4f46e5,#6366f1);border:none" @click="openAddLlm()"><span class="btn-ico" v-html="ico.plus"></span>添加模型配置</button>'
    + '</div></div>'
    + '<div class="empty-hint" v-if="!loadingLlm && llmList.length===0">暂无模型配置，点击上方「添加模型配置」或从预置模板开始</div>'
    + '<div class="llm-grid" v-else>'
    + '<div class="llm-card" v-for="m in filteredLlm" :key="m.id" :class="{\'llm-active\': m.active}">'
    + '<div v-if="m.active" class="llm-active-badge-top"><svg viewBox="0 0 24 24" width="12" height="12" fill="currentColor"><circle cx="12" cy="12" r="9"/></svg> 当前使用中</div>'
    + '<div class="llm-card-header">'
    + '<div class="llm-icon-box" :class="{\'active\': m.active}" v-text="m.name.slice(0,2)"></div>'
    + '<div class="llm-header-info">'
    + '<div class="llm-name-row">'
    + '<span class="llm-name" v-text="m.name"></span>'
    + '<span class="llm-key-badge" :class="m.has_key?\'ok\':\'warn\'" v-text="m.has_key?\'已配置 Key\':\'未填 Key\'"></span>'
    + '</div>'
    + '<div class="llm-model-tag" v-text="m.model"></div>'
    + '</div></div>'
    + '<div class="llm-card-body">'
    + '<div class="llm-body-row"><span class="llm-body-k">API Base</span><span class="llm-body-v" v-text="m.api_base"></span></div>'
    + '<div class="llm-body-row"><span class="llm-body-k">API Key</span><span class="llm-body-v" v-text="m.api_key_masked||\'未填 Key\'" :style="!m.has_key?\'color:#d97706\':\'\'"></span></div>'
    + '<div class="llm-body-row"><span class="llm-body-k">协议类型</span><span class="llm-body-v" v-text="m.provider"></span></div>'
    + '</div>'
    + '<div class="llm-desc" v-text="m.desc||\'OpenAI 兼容对话模型，支持意向判定、画像提炼与破冰话术生成。\'"></div>'
    + '<div class="llm-card-actions">'
    + '<div class="llm-action-primary">'
    + '<button v-if="!m.active" class="btn btn-small full" style="background:#4f46e5;color:#fff;font-weight:700;padding:7px 12px" @click="activateLlm(m)" :disabled="busyLlm===m.id"><span class="btn-ico" v-html="ico.check"></span>设为主模型</button>'
    + '<button v-else class="btn btn-small full" style="background:#ecfdf5;color:#047857;border:1px solid #a7f3d0;font-weight:700;padding:7px 12px" disabled><span class="btn-ico" v-html="ico.check"></span>正在作为主模型生效</button>'
    + '</div>'
    + '<div class="llm-action-group">'
    + '<button class="btn btn-small btn-ghost" style="border:1px solid #cbd5e1" @click="testLlm(m)" :disabled="testingId===m.id"><span class="btn-ico" v-html="ico.play"></span><span v-text="testingId===m.id?\'测试中…\':\'测试\'"></span></button>'
    + '<button class="btn btn-small btn-ghost" style="border:1px solid #cbd5e1" @click="openEditLlm(m)"><span class="btn-ico" v-html="ico.edit"></span>编辑</button>'
    + '<button v-if="!m.active" class="btn btn-small btn-ghost" style="color:#ef4444;border:1px solid #fecaca" @click="delLlm(m)"><span class="btn-ico" v-html="ico.trash"></span></button>'
    + '</div>'
    + '</div>'
    + '</div>'
    + '</div>'
